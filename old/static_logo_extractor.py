...
from image_processor import preserve_logo_colors
...
async def download_and_process_logo(session, logo_url, domain):
    """
    Download logo and process it to have transparent background while preserving original colors
    """
    if not logo_url:
        return None
    ...
    try:
        ...
        async with session.get(logo_url) as response:
            if response.status == 200:
                ...
                temp_path = f"{download_dir}/{domain}_temp.png"
                with open(temp_path, 'wb') as f:
                    f.write(logo_data)
                
                # Process logo: ONLY remove background, preserve all logo colors
                output_path = f"{download_dir}/{domain}.png"
                
                try:
                    # Use color-preserving processing that only removes background
                    preserve_logo_colors(temp_path, output_path)
                    print(f"✅ Processed logo for {domain} (colors preserved)")
                except Exception as e:
                    print(f"⚠️  Error processing logo for {domain}: {e}")
        ...
        print(f"❌ Error downloading logo for {domain}: {e}")
        return None

async def extract_logo_from_domain(session, domain):
    """Extract logo URL from a domain and download/process it"""
                    ...
                    for attr in ['@src', '@alt', '@class', '@id']:
                        ...
                        if val and LOGO_PATTERN.search(" ".join(val)):
                            ...
                            if src and is_valid_image(src[0]):
                                logo_url = urljoin(url, src[0])
                                # Download and process the logo
                                processed_path = await download_and_process_logo(session, logo_url, domain)
                                return logo_url
                
                # Also check meta tags for logos
                meta_selectors = [
                ...
                for selector in meta_selectors:
                    ...
                    for meta_value in meta_values:
                        if meta_value and ('logo' in meta_value.lower() or is_valid_image(meta_value)):
                            logo_url = urljoin(url, meta_value)
                            # Download and process the logo
                            processed_path = await download_and_process_logo(session, logo_url, domain)
                            return logo_url
...
async def main():
    ...
    connector = aiohttp.TCPConnector(limit=CONCURRENCY, limit_per_host=80)
